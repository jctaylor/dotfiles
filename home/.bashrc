...
# i.e. every time a shell is started
# In contrast, .profile is the run-at-login script
...
## This is irrelevant since a prompt_cmd is used below
...
##################################################################################
# Define a function to run in PROMPT_COMMAND

function prompt_cmd
{
    # Construct PS1 time, git info, error code, etc.
    last_return=$?
    if [ "$last_return" -ne 0 ]; then
        PS1="\[\e[6;41;33m\]${last_return}\[\e[m\] "
    else
        PS1=""
    fi
...
#       files and directories when using ls --color
